...
public class LoginController {
    ...
    public ApiResult login(@RequestBody Login login, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (teacherRes != null) {
            ...
            role.setPath("/");
            Cookie teacherId = new Cookie("rb_teacher_id", String.valueOf(teacherRes.getTeacherId()));
            teacherId.setPath("/");
            response.addCookie(token);
            response.addCookie(role);
        ...
        if (studentRes != null) {
            ...
            role.setPath("/");
            Cookie studentId = new Cookie("rb_student_id", String.valueOf(studentRes.getStudentId()));
            studentId.setPath("/");
            response.addCookie(token);
            response.addCookie(role);
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        role.setPath("/");
        role.setMaxAge(0);
        Cookie teacherId = new Cookie("rb_teacher_id", null);
        teacherId.setPath("/");
        teacherId.setMaxAge(0);
        Cookie studentId = new Cookie("rb_student_id", null);
        studentId.setPath("/");
